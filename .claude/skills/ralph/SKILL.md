...
- --type [type]: process only tasks at a specific phase (extract, create, connect, update, review, enrich)
...
| create | (inline record creation) | Write the record file |
| enrich | /enrich | Add content to existing record |
| connect | /connect | Find connections, update design maps |
| update | /update | Update older records with new connections |
| review | /review | Rationale quality + schema + health checks |
...
```yaml
phase_order:
  claim: [create, connect, update, review]
  enrichment: [enrich, connect, update, review]
...
Phase distribution:
  Claims:       {create: N, connect: N, update: N, review: N}
  Enrichments:  {enrich: N, connect: N, update: N, review: N}
...
Create a record for this claim in records/[claim as sentence].md
Follow record design patterns:
- YAML frontmatter with rationale (adds info beyond title), areas
- Body: 150-400 words showing reasoning with connective words
- Footer: Source (wiki link), Relevant Records (with context), Areas
...
The task file specifies which existing record to enrich and what to add.
ONE PHASE ONLY. Do NOT run connect.
```

For **connect** phase:

**Build sibling list:** Query the queue for other claims in the same batch where `completed_phases` includes "create" (record already exists). Format as wiki links.
...
Use dual discovery: design map exploration AND semantic search.
Add inline links where genuine connections exist — including sibling claims listed above.
Update relevant design map with this record.
...
This is the BACKWARD pass. Find OLDER records AND sibling claims
that should reference this record but don't.
Add inline links FROM older records TO this record.
...
Combined verification: recite (cold-read prediction test), audit (schema check),
review (per-record health).
IMPORTANT: Recite runs FIRST — read only title+rationale, predict content,
THEN read full record.
...
1. **Collect all record paths** from completed batch tasks. For each claim task with `status: "done"`, read the task file's `## Create` section to find the created record path.
...
{list of ALL record titles + paths from completed batch tasks}

Verify sibling connections exist between batch records. Add any that were missed
because sibling records did not exist yet when the earlier claim's connect ran.
...
**Why two phases?** Workers have sibling awareness (claim titles in spawn prompt) and link proactively during connect/update. But timing means some sibling records may not exist yet during a worker's connect phase. Phase B runs a single cross-connect pass after all records exist.
...
During CONNECT and UPDATE, check if your claim genuinely connects to any sibling.
If a sibling record exists in records/, link to it inline where the
...
record title, path, and claim ID. The lead needs this for cross-connect.
```
...
1. **Parse completion message** — extract the created record title and path (needed for Phase B)
2. **Log any learnings** from the worker's report
3. **Check for issues** — failures, skipped phases, resource conflicts

**Collect all created records** — maintain a list of `{record_title, record_path}` from worker completion messages. You need this for the cross-connect validation phase.
...
Records created in this batch:
{list of ALL newly created record titles with paths from Phase A}

Verify sibling connections exist between these records. Add any connections that
workers missed because sibling records did not exist yet when a worker's connect ran.
Check backward link gaps. Output RALPH HANDOFF block when done.",
  description = "cross-connect: batch {BATCH}"
)
```
...
Queue state:
  Pending: {count}
  Done: {count}
  Phase distribution: {create: N, connect: N, update: N, review: N}
...
After each phase, the task file's corresponding section (Create, Connect, Update, Review) should be filled. If empty after subagent completes, log warning.
...
- Include sibling claim titles in connect and update prompts
