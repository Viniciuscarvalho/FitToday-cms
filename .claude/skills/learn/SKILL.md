...
ops/derivation-manifest.md  — domain vocabulary (inbox folder, document skill name)
```
...
- Domain name and hub design map name
...
```yaml
---
rationale: [1-2 sentence summary of key findings]
...
areas: ["[[domain-hub-design-map]]"]
---
```

Include only the fields relevant to the tool used:
- Deep researcher: `source_type`, `exa_prompt`, `exa_research_id`, `exa_model`, `generated`, `domain`, `areas`
- Exa web search: `source_type`, `exa_prompt`, `exa_tool`, `generated`, `domain`, `areas`
- WebSearch: `source_type`, `exa_prompt`, `exa_tool`, `generated`, `domain`, `areas`
...
Research complete
  ...
  Next: /document inbox/[filename]
```

Append based on mode:
- **manual:** (nothing extra)
- **suggested:** `Ready for processing when you are.`
- **automatic:** Replace "Next" line with `Queued for /document -- processing will begin automatically.`
...
| Phase | Skill | Purpose |
|-------|-------|---------|
| Extract insights | /document | Mine research for atomic propositions |
| Find connections | /connect | Link new insights to existing graph |
| Update old records | /update | Backward pass on touched records |
| Quality check | /review | Rationale quality, schema, links |
