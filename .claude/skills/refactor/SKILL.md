...
|--------|-------------------|-------------|
| **Granularity shift** | Record templates, extraction depth in /document, processing skills, context file "Record Design" section | Template body length guidance, extraction granularity settings, composability test thresholds |
| **Organization shift** | Folder structure, design map hierarchy, context file "Folder Architecture" section, hub design map | Directory layout, design map tier count, navigation references |
...
| **Navigation shift** | Design map tier structure, hub design map, context file "Design Map" section, record Areas footers | Number of design map tiers, hub content, navigation instructions |
| **Maintenance shift** | /health threshold values, condition-based trigger settings, context file maintenance instructions | Check frequency conditions, stale record thresholds, update trigger conditions |
| **Schema shift** | Templates (_schema blocks), validation rules, /audit skill, query scripts, context file "YAML" section | Required fields, enum values, validation patterns |
...
Artifact: [file path]
  ...
  Content impact: [does this affect existing records? how many?]
```

### Content Impact Assessment

Some changes affect only infrastructure (skills, templates, context file). Others affect existing content (records, design maps):

| Change Type | Content Impact | Action Required |
|-------------|---------------|----------------|
| Template field addition | New records get field, old records don't | Add field to old records (schema migration) |
| Template field removal | Old records have unused field | Remove field from old records (optional) |
| Folder restructure | Records must move | Move files, update all wiki links |
| Design map tier change | Design map hierarchy restructured | Merge/split design maps, update Areas footers |
| Enum value change | Old records have invalid values | Update values in affected records |

For content-impacting changes:
- Count affected records: `grep -rl '[old value]' records/*.md | wc -l`
...
Detected [N] dimension changes:
  ...
  [dimension]: [old] -> [new]
    ...
    Content impact: [N] existing records need [what]
    Risk: [low/medium/high]
...
|-----------------|---------------------|
| Granularity | /document, /review, /audit |
| Processing | /document, /connect, /update, /review, /ralph, /pipeline |
| Linking | /connect, /update |
| Navigation | /connect (design map update logic) |
| Schema | /audit, /review |
...
1. **Schema migration:** Add/remove/rename fields across records
   ```bash
   # Example: add a new required field to all records
   for f in records/*.md; do
     grep -q '^new_field:' "$f" || sed -i '' '/^rationale:/a\
...
4. **Design map restructuring:** Merge/split design maps as needed
   - For splits: create sub-design-maps, move Core Ideas, update parent
   - For merges: combine Core Ideas, remove redundant design map, update records' Areas footers
...
| Schema compliance | Check all records against template _schema blocks | All required fields present, enum values valid |
| Design map hierarchy | Verify hub links to domain design maps, domains link to areas | All tiers connected |
| Session orient | Simulate session start — can the context file be loaded? | No file-not-found errors |
| Three-space boundaries | Check records vs ops vs self boundaries | No content in wrong space |
...
--=={ refactor complete }==--
  ...
  Content migration:
    [N] records updated with new schema fields
    [N] wiki links updated
    [N] design maps restructured
...
### Large Content Migration (100+ records)

For large migrations:
1. Show the count and estimated time
2. Offer batch processing: "This affects 150 records. Process all at once or in batches of 50?"
