...
# Record count (excluding design maps)
...
# Connection count (all wiki links across records/)
LINK_COUNT=$(grep -ohP '\[\[[^\]]+\]\]' "$NOTES_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ')

# Average connections per record
...
# Area count (unique values in areas: fields)
...
```bash
# Orphan count (records with zero incoming links)
ORPHAN_COUNT=0
...
# Schema compliance (% of records with required fields: rationale, areas)
MISSING_DESC=$(grep -rL '^rationale:' "$NOTES_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ')
MISSING_TOPICS=$(grep -rL '^areas:' "$NOTES_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ')
SCHEMA_ISSUES=$((MISSING_DESC + MISSING_TOPICS))
if [[ "$TOTAL_FILES" -gt 0 ]]; then
  # Records with BOTH required fields
...
# Processed ratio (records vs inbox)
...
```bash
# This week's growth (records with created: date within last 7 days)
...
# This week's connections (approximate — count links in recently created records)
...
--=={ stats }==--
  ...
  ===============
  records:       [NOTE_COUNT]
  Connections:   [LINK_COUNT] (avg [AVG_LINKS] per record)
  design maps:   [MOC_COUNT] (covering [COVERAGE]% of records)
  Areas:         [TOPIC_COUNT]
  ...
  Growth
  ======
  This week:    +[THIS_WEEK_NOTES] records, +[THIS_WEEK_LINKS] connections
...
| Condition | Note |
|-----------|------|
| ORPHAN_COUNT > 0 | "[N] orphan records — run `/graph health` for details" |
| DANGLING_COUNT > 0 | "[N] dangling links — run `/graph health` to identify broken links" |
| COMPLIANCE < 90 | "Schema compliance below 90% — some records missing required fields" |
| OBS_PENDING >= 10 | "[N] pending observations — consider running /reassess" |
| TENSION_PENDING >= 5 | "[N] open tensions — consider running /reassess" |
| DENSITY < 0.02 | "Graph density is low — connections are thin. Run /connect to strengthen the network" |
| PROCESSED_PCT < 50 | "More content in inbox than in records/ — consider processing backlog" |
| THIS_WEEK_NOTES == 0 | "No new records this week" |
...
- **[NOTE_COUNT]** records with **[LINK_COUNT]** connections (avg [AVG_LINKS] per record)
- **[MOC_COUNT]** design maps covering [COVERAGE]% of records
...
Use universal vocabulary (records, design maps, etc.). All metrics work identically.

### Very Large Vault (500+ records)

The orphan and design map coverage checks may be slow for large vaults. If records/ has >200 files:
1. Run orphan detection with a simpler heuristic (check only for presence in any design map, not full backlink scan)
