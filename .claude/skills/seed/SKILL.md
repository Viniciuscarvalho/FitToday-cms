...
Or via keyword search in the records/ directory:
```bash
grep -rl "{key terms from source title}" records/ 2>/dev/null | head -5
```
...
**Why move immediately:** All references (task files, records' Source footers) use the final archived path from the start. No path updates needed later. If it is in inbox, it is unclaimed. Claimed sources live in archive.
...
# Extract records from {source filename}
...
- Duplicate check against records/ during extraction
