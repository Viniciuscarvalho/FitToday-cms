---
name: pipeline
description: End-to-end source processing -- seed, document, process all claims through connect/update/review, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Phase 2: /document (via /ralph) — extract claims from source
    |
    v
Phase 3: /ralph (all claims) — create -> connect -> update -> review
    |
    v
...
Report:
```
$ Extracted: {N} records, {M} enrichments
...
This processes every claim through: create -> connect -> update -> review. And every enrichment through: enrich -> connect -> update -> review.
...
```
$ Processing record 1/{total}: {title}
  $ create... done
  $ connect... done (3 connections found)
  $ update... done (2 records updated)
  $ review... done (PASS)
```
...
- List of created records with titles
...
Processing:
  records created: {N}
  Existing records enriched: {M}
  Connections added: {C}
  design maps updated: {T}
  Older records updated via update: {R}

Quality:
  All review checks: {PASS/FAIL count}
...
- Extracted {N} records and {M} enrichments
...
- Show the user what was created (list of records)
