---
name: update
description: Update old records with new connections. The backward pass that /connect doesn't do. Revisit existing records that predate newer related content, add connections, sharpen claims, consider splits. Triggers on "/update", "/update [record]", "update old records", "backward connections", "revisit records".
user-invocable: true
...
| Depth | Update Behavior |
|-------|-----------------|
| deep | Full reconsideration. Search extensively for newer related records. Consider splits, rewrites, challenges. Evaluate claim sharpening. Multiple search passes. |
| standard | Balanced review. Search semantic neighbors and same-design map records. Add connections, sharpen if needed. |
...
| Scope | Behavior |
|-------|----------|
| related | Search records directly related to the target (same design map, semantic neighbors) |
| broad | Search across all design maps and semantic space for potential connections |
...
Parse immediately:
- If target contains `[[record name]]` or record name: update that specific record
- If target contains `--handoff`: output RALPH HANDOFF block at end
- If target is empty: find records that most need updating (oldest, sparsest, most outdated)
- If target is "recent" or "--since Nd": update records not touched in N days
- If target is "sparse": find records with fewest connections

**Execute these steps:**

1. **Read the target record fully** — understand its current claim, connections, and age
2. **Ask the update question:** "If I wrote this record today, with everything I now know, what would be different?"
3. **If a task file exists** (pipeline execution): read it to see what /connect discovered. The Connect section shows which connections were just added and which design maps were updated — this is your starting context for the backward pass.
4. **Search for newer related records** — use dual discovery (semantic search + design map browsing) to find records created AFTER the target that should connect
5. **Evaluate what needs changing:**
   - Add connections to newer records that did not exist when this was written
   - Sharpen the claim if understanding has evolved
   - Consider splitting if the record now covers what should be separate ideas
   - Challenge the claim if new evidence contradicts it
   - Rewrite prose if understanding is deeper now
6. **Make the changes** — edit the record with new connections (inline links with context), improved prose, sharper claim if needed
7. **Update design maps** — if the record's area membership changed, update relevant design maps
8. **If task file exists:** update the update section
...
Revisit old records with everything you know today. Records are living documents — they grow, get rewritten, split apart, sharpen their claims. This is the backward pass that keeps the network alive.

## Philosophy

**Records are living documents, not finished artifacts.**

A record written last month was written with last month's understanding. Since then:
- New records exist that relate to it
- Understanding of the area deepened
...
Updating is not just "add backward links." It is completely reconsidering the record based on current knowledge. Ask: **"If I wrote this record today, what would be different?"**

> "The record you wrote yesterday is a hypothesis. Today's knowledge is the test."
...
| **Split the record** | Multiple claims bundled together |
| **Challenge the claim** | New evidence contradicts the original |
| **Improve the rationale** | Better framing emerged |
| **Update examples** | Better illustrations exist now |

Updating is NOT just Phase 4 of /connect applied backward. It is a full reconsideration.

## Invocation Patterns

### /update [[record]]

Fully reconsider a specific record against current knowledge.

### /update (no argument)

Scan for candidates needing updating, present ranked list.

### /update --sparse

Process records flagged as sparse by /health.

### /update --since Nd

Update all records not modified in N days.

**How to find candidates:**
```bash
# Find records not modified in 30 days
find records/ -name "*.md" -mtime +30 -type f
```
...
### Phase 1: Understand the Record as It Exists

Read the target record completely. Understand:
...
- Which design maps were updated
...
Use the same dual discovery pattern as /connect — design map exploration AND semantic search in parallel.

**Path 1: Design Map Exploration** — curated navigation

From the record's Areas footer, identify which design map(s) it belongs to:
- Read the relevant design map(s)
- What synthesis exists that might affect this record?
- What newer records in Core Ideas should this record reference?
- What tensions involve this record?
...
**Also check:**
- Backlinks — what records already reference this one? Do they suggest the target should cite back?

```bash
grep -rl '\[\[target record title\]\]' records/ --include="*.md"
```

**Key question:** What do I know today that I did not know when this record was written?
...
| Claim is too broad | Split into focused records |
...
Does this record make multiple claims that could stand alone?
- If the record connects to 5+ areas across different domains, it probably needs splitting
- If you would want to link to part of it but not all, it is a split candidate

### Phase 4: Evaluate Connections

**Backward connections (what this record should reference):**

For each newer record, ask:
- Does it extend this record's argument?
...
**Forward connections (what should reference this record):**

Check newer records that SHOULD link here but do not:
...
**For interactive execution (no --handoff):** Present the update proposal first, then apply after approval.

**Update proposal format (interactive only):**

```markdown
## Update Proposal: [[target record]]

**Last modified:** YYYY-MM-DD
**Current knowledge evaluated:** N newer records, M backlinks
...
**2. [change type]: [rationale]**
...

### Connections to Add

- [[newer record A]] — [relationship]: [specific reason]
- [[newer record B]] — [relationship]: [specific reason]

### Connections to Verify (other records should link here)

- [[record X]] might benefit from referencing this because...
...
5. Update rationale if claim changed

---

## The Five Update Actions

### 1. Add Connections

The simplest action. Newer records exist that should be referenced.
...
```yaml
relevant_records:
  - "[[newer record]] — extends this by adding temporal dimension"
```
...
- Rationale (must match new claim)
- Body (reasoning must support sharpened claim)

### 4. Split the Record

One record became multiple ideas over time. Splitting creates focused, composable pieces.

**Split indicators:**
- Connects to 5+ areas across different domains
...
2. Create new records for each claim
3. Each new record gets:
   - Focused title (the claim)
   - Own rationale
   - Relevant subset of content
   - Appropriate connections
4. Original record either:
...
# if tension exists
This argues [X]. But [[contradicting record]] argues [Y]. The tension remains unresolved — possibly [X] applies in context A while [Y] applies in context B.

# if significantly wrong
This record originally claimed [X]. Based on [[evidence]], the claim is revised: [new claim].
```

**Always log challenges:** When a claim is challenged or revised, this is a significant event. Note it in the task file Update section with the original claim, the new evidence, and the revised position.

---

## Enrichment-Triggered Actions

When processing a record that came through the enrichment pipeline, check the task file for `post_enrich_action` signals. These were surfaced by /enrich and need execution:

### title-sharpen

The enrich phase determined the record's title is too vague after content integration.
...
4. Update the record's rationale to match the new title
5. Log the rename in the task file Update section

### split-recommended

The enrich phase determined the record now covers multiple distinct claims.
...
3. If yes:
   - Create new record files for each split claim
   - Move relevant content from original to splits
   - Update original to either link to splits or retain one claim
   - Create queue entries for the new records starting at the connect phase
4. Log the split in the task file Update section

### merge-candidate

The enrich phase determined this record substantially overlaps with another.

**Do NOT auto-merge or auto-delete.** This requires human judgment.

1. Log the merge recommendation in the task file Update section
2. Note which records overlap and why
...
After changes, is the record better? More useful? More connected? More accurate?

If you cannot confidently say yes, do not make the change.

### Gate 3: Coherence Test

After changes, does the record still cohere as a single focused piece? Or did you accidentally make it broader?
...
- The record is accurate, well-connected, and recent — leave it alone
- The "improvement" would just be cosmetic rewording — do not churn
- The record is a historical record — these evolve through status changes, not rewrites
...
```markdown
## Update Complete: [[target record]]

### Changes Applied

| Type | Rationale |
|------|-----------|
| connection | added [[record A]] inline, [[record B]] to footer |
| rewrite | clarified reasoning in paragraph 2 |
| sharpen | title unchanged, rationale updated |
...
- Outgoing links: 3 -> 5
- This record now bridges [[domain A]] and [[domain B]]

### Cascade Recommendations

- [[related record]] might benefit from update (similar vintage)
- Design map [[area]] should be updated to reflect changes
...
Successful updating:
- Record reflects current understanding, not historical understanding
- Claim is sharp enough to disagree with
- Connections exist to relevant newer content
- Record participates actively in the network
- Someone reading it today gets the best version

The test: **if this record were written today with everything you know, would it be meaningfully different?** If yes and you did not change it, updating failed.
...
- Split records into pieces too thin to stand alone
- Add connections without articulating why
- Rewrite voice/style (preserve the record's character)
...
Records written yesterday do not know about today. Records written with old understanding do not reflect new understanding. Without updating, the vault becomes a graveyard of outdated thinking that happens to be organized.

Updating is how knowledge stays alive. Not just connecting, but questioning, sharpening, splitting, rewriting. Every record is a hypothesis. Every update is a test.
...
Work Done:
- Older records updated: N
- Claim status: unchanged | sharpened | challenged | split
- Network effect: M new traversal paths

Files Modified:
- records/[older record 1].md (inline link added)
- records/[older record 2].md (footer connection added)
- [task file path] (update section)
...
Queue Updates:
- Advance phase: update -> review
=== END HANDOFF ===
```

### Task File Update (when invoked via ralph loop)

When running in handoff mode via /ralph, the prompt includes the task file path. After completing the workflow, update the `## update` section of that task file with:
- Older records updated and why
- Claim status (unchanged/sharpened/challenged/split)
- Network effect summary

**Critical:** The handoff block is OUTPUT, not a replacement for the workflow. Do the full update workflow first, update task file, then format results as handoff.
...
# NEXT_PHASE is the phase after update in phase_order (i.e., review)
jq '(.tasks[] | select(.id=="TASK_ID")).current_phase = "review" |
    (.tasks[] | select(.id=="TASK_ID")).completed_phases += ["update"]' \
...
After updating completes, output the next step based on `ops/config.yaml` pipeline.chaining mode:

- **manual:** Output "Next: /review [record]" — user decides when to proceed
