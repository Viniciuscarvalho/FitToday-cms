...
1. **`ops/derivation-manifest.md`** — vocabulary mapping, domain context
   ...
   - Use `vocabulary.rethink` for reassess command name
...
Read `ops/derivation-manifest.md` (or fall back to `ops/derivation.md`) for domain vocabulary mapping. All output must use domain-native terms. If neither file exists, use universal terms (records, inbox, design map, etc).
...
|-----------|------------------|
| orphan_notes | For each record in records/, count incoming [[links]]. Zero = orphan. |
...
| moc_oversize | For each design map, count linked records. |
| stale_notes | Records not modified in 30+ days with < 2 links. |
| low_link_density | Average link count across all records. |
| methodology_drift | Compare config.yaml modification time vs newest ops/methodology/ record modification time. If config is newer, methodology may be stale. |
...
| **Queue state** | Read `ops/queue.yaml` or `ops/queue/queue.json` — pending pipeline tasks | Total pending, by phase (create, connect, update, review), blocked phases |
| **Inbox pressure** | Count `*.md` files in inbox/, find oldest by mtime | Count per subdirectory, age of oldest item in days |
| **Record count** | Count `*.md` in records/ | Total records for context |
| **Orphan records** | For each record, grep for `[[filename]]` across all files — zero hits = orphan | Count, first 5 names |
| **Dangling links** | Extract all `[[links]]` from records/, verify each target file exists | Count, first 5 targets |
| **Stale records** | Records not modified recently AND with low link density (< 2 links) | Count |
...
# Record count
NOTE_COUNT=$(ls -1 records/*.md 2>/dev/null | wc -l | tr -d ' ')
...
|-------|---------|-----------|-------------------|
| **Session** | Inbox > 5 items, orphan records (any), dangling links (any), 10+ pending observations, 5+ pending tensions, unprocessed sessions > 3 | Immediate — these degrade work quality right now | Orphans are invisible to traversal. Dangling links confuse navigation. Inbox pressure means lost ideas. Observation/tension thresholds mean the system is accumulating unprocessed friction. |
| **Multi-session** | Pipeline queue backlog > 10, research gaps identified in goals, stale records > 10, inbox items aging > 7 days, methodology captures > 5 in same category | Soon — these compound over days | Unfinished pipeline batches block downstream connections. Stale records represent decaying knowledge. Aging inbox means capture is outpacing processing. |
| **Slow** | Health check not run in 14+ days, design map oversized (>40 records), link density below 2.0 average, low record count relative to time | Background — annoying but not blocking | These are maintenance tasks. Important for long-term health but not urgent. |

**Threshold rule:** 10+ pending observations OR 5+ pending tensions is ALWAYS session-priority. Recommend /reassess in this case.
...
- orphan_notes: "{N} records invisible to traversal"
...
|--------|---------------|-------------------|
| Dangling links / orphans | /health or specific fix command | "You have [N] orphan records invisible to traversal. Connecting them increases graph density and retrieval quality." |
| 10+ observations or 5+ tensions | /reassess | "[N] pending observations have accumulated. Pattern detection requires processing this backlog to evolve the system." |
| Inbox > 5 items | /document [specific file] | "Your inbox has [N] items (oldest: [age]). [File X] has the highest connection potential based on [reason]." |
| Unprocessed sessions > 3 | /flag --mine-sessions | "[N] sessions have uncaptured friction patterns. Mining them prevents methodology regressions." |

**When recommending inbox processing:** Choose the specific inbox item that aligns best with current goals or has the most connection potential to existing records. Recommend a concrete file, not "process some inbox."
...
| Signal | Recommendation | Rationale Template |
|--------|---------------|-------------------|
| Queue backlog > 10 | /ralph [N] | "[N] pipeline tasks are pending. Your newest records lack connections, which means they can't participate in synthesis." |
| Stale records > 10 | /update [specific record] | "[N] records haven't been touched since [date]. [Record X] has the most connections and would benefit most from updating." |
| Research gaps | /document [file aligned with goals] | "Your goals mention [topic] but your graph has few records there. [Inbox item] addresses this gap." |
| Methodology convergence | /reassess | "[N] methodology captures in the [category] area suggest a pattern worth elevating." |

**When recommending updating:** Choose the most-connected stale record (highest link density + oldest modification). Updating high-connectivity records has the highest ripple effect.
...
| Design map oversized | Restructuring suggestion | "[Design map X] has [N] records. Splitting into sub-design-maps improves navigation and reduces cognitive load." |
| Low link density | /update on lowest-density record | "Your graph has an average link density of [N]. Updating sparse records increases traversal paths." |
...
next
  ...
  or update older records to deepen the graph.
```
...
| Good | Bad |
|------|-----|
| `/document inbox/article-on-spaced-repetition.md` | "process some inbox items" |
| `/ralph 5` | "work on the queue" |
| `/reassess` | "review your observations" |
| `/update [[record title here]]` | "update some old records" |
...
**State:** Inbox: [N] | Records: [N] | Orphans: [N] | Dangling: [N] | Stale: [N] | Obs: [N] | Tensions: [N] | Queue: [N]
**Recommended:** [action]
...
3. /reassess evidence — persistent recommendations that go unacted-on may reveal misalignment between what the system detects and what the user values

---

## Edge Cases

### Empty Vault (0-5 records)

Recommend capturing or documenting content. Maintenance is premature with < 5 records — the graph does not have enough nodes for meaningful analysis.

```
next

  State:
    Records: [N] — early stage vault

  Recommended: Capture or /document content
  Rationale: Your graph has [N] records. At this stage, adding
  content matters more than maintaining structure. Health checks,
  updating, and reassess become valuable after ~10 records.
```

### Everything Clean

Say so explicitly. Recommend exploratory work aligned with goals, or reflective work on older records:

```
  No urgent work detected. Consider:
  - Exploring a research direction from goals.md
  - Updating older records to deepen connections
...
- Observation threshold -> reassess prevents methodology drift
...
| Vague recommendations | "Process inbox" gives no actionable starting point | Name the specific file, record, or command |
...
| Recommending maintenance too early | A 5-record vault does not need health checks | Scale recommendations to vault maturity |
