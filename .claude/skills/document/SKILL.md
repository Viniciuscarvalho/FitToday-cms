...
You are the extraction engine. Raw source material enters. Structured, atomic records exit. Everything between is your judgment — and that judgment must err toward extraction, not rejection.

### The Core Distinction

| Concept | What It Means | Example |
|---------|---------------|---------|
| **Having knowledge** | The vault contains information | "We store records in folders" |
| **Articulated reasoning** | The vault explains WHY something works as a traversable record | "folder structure mirrors cognitive chunking because..." |
...
1. **Extract ALL core records** — direct assertions about the domain that can stand alone as atomic propositions.

2. **Extract ALL evidence and validations** — if source confirms an approach, that confirmation IS the record. Evidence is extractable even when the conclusion is already known, because the reasoning path matters.
...
5. **Extract ALL enrichments** — if source adds detail to existing records, create enrichment tasks. Near-duplicates almost always add value.
...
**"Would a future session benefit from this reasoning being a retrievable record?"**
...
- "already captured in system config" — config is implementation, not articulation. The WHY needs a record.
...
- Completely off-topic (unrelated to the domain)
...
4. For each candidate:
   - Tier 1 (preferred): use `mcp__qmd__vector_search` with query "[claim as sentence]", collection="records", limit=5
   - Tier 2 (CLI fallback): `qmd vsearch "[claim as sentence]" --collection records -n 5`
...
| Tension: content contradicts existing record | Create atomic note in `ops/tensions/` with prose-sentence title |
...
Extract composable records from source material into records/.
...
|---------|---------|-----------------|
| **We DO this** | "We tag records with areas" | — (not sufficient) |
| **We explain WHY** | "area tagging enables cross-domain navigation because..." | This |
...
- BETTER thinking: "Does this serve the domain?"
- BEST thinking: **"Would a future session benefit from this reasoning being a retrievable record?"**
...
**THE RULE:** Implementation without articulation is incomplete. If we DO something but lack a record explaining WHY it works, that articulation needs extraction.
...
|----------|--------------|-------------|----------------|
| Core domain records | Direct assertions about the domain | record | NO |
| Patterns | Recurring structures across sources | record | NO |
| Comparisons | How different approaches compare, X vs Y, trade-offs | record | NO |
...
| Enrichments | Content that adds detail to existing records | enrichment task | NO |
| Open questions | Unresolved questions worth tracking | record (open) | NO |
| Implementation ideas | Techniques, workflows, features to build | methodology note | NO |
| Validations | Evidence confirming an approach works | record | NO |
...
- Named methods: any named system, technique, or framework relevant to the domain
...
**Enrichment signals:**
- Content covering ground similar to an existing record
...
For EVERY candidate, ask: **"Does this serve the domain?"**
...
**CRITICAL:** This gate exists to filter OUT content that does not serve the domain. It applies ONLY to standard claims from GENERAL (off-topic) sources.
...
The claim is understandable without source context. Someone reading this record cold can grasp what it argues without needing to know where it came from.
...
This record would be linked FROM elsewhere. Records function as APIs. If you cannot imagine writing `since [[this claim]]...` in another record, it is not composable.
...
Not already captured in the vault. Semantic duplicate check AND existing records scan both clear.

Fail: semantically equivalent to an existing record
...
Pass: extends, contradicts, or deepens existing records
...
```bash
# Get rationales from existing records
for f in records/*.md; do
  [[ -f "$f" ]] && echo "=== $(basename "$f" .md) ===" && rg "^rationale:" "$f" -A 0
done
```

Scan rationales to understand current records. This prevents duplicate extraction and helps identify connection points and enrichment opportunities.
...
- How many records do you expect from this source?
...
|----------|-----------------|----------|
| Core domain record | Direct assertion about the domain | -> record (SKIP selectivity gate) |
...
| Validation | Evidence confirming an approach works | -> record (SKIP selectivity gate) |
| Near-duplicate | Semantic search finds related vault record | -> evaluate for enrichment task |
| Off-topic claim | General insight not about the domain | -> apply selectivity gate |

**CRITICAL:** Implementation ideas, tensions, validations, and domain records do NOT need to pass the 4-criterion selectivity gate. The gate is for off-topic filtering ONLY.
...
```
mcp__qmd__vector_search  query="[proposed claim as sentence]"  collection="records"  limit=5
```
If MCP is unavailable, run:
```bash
qmd vsearch "[proposed claim as sentence]" --collection records -n 5
```
...
3. Ask: **"What does source add that existing record lacks?"**
...
| Same claim, different words, source adds nothing | SKIP (verify by re-reading existing record) |
| Same claim, source has MORE detail/examples/framing | -> ENRICHMENT TASK (update existing record) |
| Same topic, DIFFERENT claim | -> EXTRACT as new record, flag for cross-linking |
| Related mechanism, different scope | -> EXTRACT as new record, flag for cross-linking |
...
1. **READ the existing record fully** (not just title/rationale)
2. Ask: "What does source ADD that existing record LACKS?"
...
CLAIMS (records):
1. [claim as sentence] — connects to [[existing record]]
2. [claim as sentence] — extends [[existing record]]
...
...
ENRICHMENT TASKS (update existing records):
1. [[existing record]] — source adds [what is missing]
...

SKIPPED (truly nothing to add):
- [rationale] — why nothing extractable
```
...
Test: "this record argues that [title]"
...
```markdown
---
rationale: [~150 chars elaborating the claim, adds info beyond title]
...
- Title passes the claim test ("this record argues that [title]")
- Rationale adds information beyond the title (not a restatement)
- Body shows reasoning, not just assertion
- At least one relevant record connection identified
- At least one design map link
- Source attribution present

**d. Create the file**

Write to: `records/[title].md`
...
1. Keep a running list of extracted records across chunks
2. Later chunks check against earlier chunks' extractions (not just existing vault records)
...
When source content adds value to an EXISTING record rather than creating a new one, create an enrichment task instead.

### When to Create Enrichment Tasks

| Signal | Action |
|--------|--------|
| Source has better examples for an existing record | Enrichment: add examples |
...
- **Target:** Which existing record to enrich (by title)
- **What to add:** Specific content from the source
- **Why:** What the existing record lacks that this adds
- **Source lines:** Where in the source the enrichment content is found

**The enrichment default:** When in doubt between "new record" and "enrichment to existing record", lean toward enrichment. The existing record already has connections, design map placement, and integration. Adding to it compounds existing value.
...
1. **"validates existing approach" as skip reason**
   ...
   - RIGHT: Validations ARE valuable. Extract as record with evidence framing.
...
2. **"already captured in system config" as skip reason**
   ...
   - WHY: Config is implementation. Records explain WHY it works.
...
5. **Treating near-duplicates as skips instead of enrichments**
   - WRONG: "Similar to existing record, skip"
   - RIGHT: Create enrichment task to add source's details to existing record
...
Before skipping ANYTHING, ask: **"Would a future session benefit from this being a retrievable record?"**
...
- Creating records without articulating vault connection
- Titles that are areas, not claims ("knowledge management" instead of "knowledge management fails without active maintenance")
...
1. **Did any "duplicates" have source content that enriches existing records?**
   - YES -> convert to enrichment task (DEFAULT TO ENRICHMENT)
   - NO -> verify by re-reading existing record FULLY
...
5. **Did any "rejected" items suggest questions worth investigating?**
   - YES -> extract as open question record
...
The claim test: "this record argues that [title]"
...
The rationale is progressive disclosure: title says WHAT the claim is, rationale says WHY it matters or HOW it works. If the rationale just rephrases the title, it wastes context and provides no filter value.
...
- Invokes other records as prose
...
- Design map notes (sections: Synthesis, Core Ideas, Tensions, Explorations Needed, Agent Notes)
- Implementation patterns with discrete steps
- Records exploring multiple facets of a concept (>1000 words AND distinct sub-topics)

**Use prose without headings for:**
- Single flowing arguments under ~1000 words
- Records where transitions like "since [[X]]..." already carry structure
...
Before finalizing ANY record, verify:

**1. Standalone Sense**
If you link to this record from another context, will it make sense without reading three other records first?

**2. Specificity**
Could someone disagree with this claim? Vague records cannot be built on.

**3. Clean Linking**
Would linking to this record drag unrelated content along? If yes, the record covers too much.
...
- Each created record's Source footer links to the source file
- The source file's YAML contains the research prompt
- The chain: research query -> inbox file -> /document -> records
...
**Source:** 300-line research document directly relevant to the domain
...
- 5 enrichment tasks -> update existing records
...
**Source:** 100-line article with partial relevance to the domain
...
**The principle:** the goal is to capture everything relevant to the domain. For domain-relevant sources, that is MOST of the content. The selectivity gate exists for OFF-TOPIC filtering, not for rejecting on-mission content that happens to have a different form.
...
semantic_neighbor: "[related record title]" | null
---
...
---
type: enrichment
target_note: "[[existing record title]]"
source_task: [source-basename]
addition: "what to add from source"
source_lines: "NNN-NNN"
---

# Enrichment EEE: [[existing record title]]

Source: [[source filename]] (lines NNN-NNN)

## Document Notes

Enrichment for [[existing record title]]. Source adds [what it adds].
...
| Record just created | /connect | New records need connections |
| After connecting | /update | Old records need updating |
...
- **manual:** Output "Next: /connect [created records]" — user decides when to proceed
- **suggested:** Output next step AND add each created record to `ops/queue/queue.json` with `current_phase: "create"` and `completed_phases: []`
